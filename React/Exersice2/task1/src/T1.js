...
export default function T1() {
  ...
  const handleDelete = (index) => {
    if (editClick && editIndex === index) {
      // Check if deleting the row currently being edited
      setEditClick(false); // Exit edit mode
      setEditIndex("");
    } else {
      // Delete the row
  ...
  const handleEdit = (index) => {
    ...
    setInputs({
      // Set input fields with the data of the selected row for editing
      ...
      <div className="formdata">
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ width: "100%" }}>
            ...
            {editClick} {/* Display 'Update' or 'Add' based on edit mode */}
          </div>
        </form>
      </div>
